...
**Implementation.** A reference d*-separation algorithm is provided in `minimal_model/dseparation.py`; T1's predictions on the worked example are verified by `minimal_model/test_dseparation.py` (14 tests passing, including the deterministic-augmentation case $A \perp D \mid C$).
